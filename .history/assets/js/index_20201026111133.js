...
let title = "Développeur d'application web et mobile";
let myText = "Création de sites web";
let myArr = title.split("");
let myArray = myText.split("");
let timeLoope;
let timeLooper;

function loop() {
    if(myArray.length > 0 && title.length > 0){
        document.getElementById('titleh1').innerHTML += myArr.shift();
        document.getElementById('textMachine').innerHTML += myArray.shift();
    } else {
        clearTimeout(timeLoope);
        clearTimeout(timeLooper);
    }
    timeLoope = setTimeout('loop()', 70);
